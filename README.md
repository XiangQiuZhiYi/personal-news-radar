...
页面支持使用浏览器语音引擎连续朗读当前分类，也可以在新闻卡片上单独朗读一条。连续播放会先读当天概览，再读取每条新闻的标题、摘要和“对普通人的影响”；影响分析会先标明“有利 / 不利 / 分化 / 当前不变”，再给出一句明确的变化结论，以及受影响人群、影响路径、短期与中长期变化、可采取行动和不确定性。评分、来源、标签、“为什么值得看”和其他界面文字不会进入应用的语音队列。
